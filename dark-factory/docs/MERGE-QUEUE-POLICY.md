...
`dev` before entering the queue. The queue itself ensures
consistency.
...
```bash
gh pr create --title "type(scope): description [ASP-XXX]" --body "..."
```
...
- `git push origin dev` -- blocked by branch protection
...
- PR titles MUST follow: `type(scope): description [ASP-XXX]`
