...
4. **Readiness gate**: verifies merge queue enforcement on `dev`
...
./dark-factory/scripts/factory-start.sh story ASP-456

# Feature-level work (4-5 agents: TDM + BE + FE + QAS + RTE)
./dark-factory/scripts/factory-start.sh feature ASP-123

# Epic-level work (6-8 agents: full SAFe team)
./dark-factory/scripts/factory-start.sh epic ASP-789
```

The session name follows the pattern `factory-ASP-XXX` (or
...
./dark-factory/scripts/factory-attach.sh factory-ASP-123 4
...
tail -20 ~/.dark-factory/logs/factory-ASP-123/qas.log
...
./dark-factory/scripts/factory-stop.sh factory-ASP-123
...
tail -f ~/.dark-factory/logs/factory-ASP-123/*.log

# Tail a specific agent
tail -f ~/.dark-factory/logs/factory-ASP-123/be-developer.log

# Search logs for errors
grep -i "error\|fail\|blocked" ~/.dark-factory/logs/factory-ASP-123/*.log
```
...
./dark-factory/scripts/factory-attach.sh factory-ASP-123

# Attach with a specific pane selected (e.g., pane 2 = BE Developer)
./dark-factory/scripts/factory-attach.sh factory-ASP-123 2
```
...
tmux attach -t factory-ASP-123 -r
```
...
./dark-factory/scripts/factory-attach.sh factory-ASP-123 3
...
- Merge queue prevents direct pushes to `dev`
...
No agent ever runs `git push` to `dev` or `gh pr merge` without
