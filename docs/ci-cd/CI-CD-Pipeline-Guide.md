...
- ESLint with a-safe-pulse standards
...
```bash
# Payment features
/app/api/payments/ @payments-team cheddarfox
/lib/stripe/ @payments-team cheddarfox

# Authentication
/app/api/auth/ @auth-team cheddarfox
/middleware.ts @auth-team cheddarfox
...
- ASP-1X: Payments team
- ASP-2X: Auth team
- ASP-3X: Frontend team
...
git checkout -b ASP-15-stripe-checkout-integration

# Make changes and commit with SAFe format
git commit -m "feat(payments): add Stripe checkout flow [ASP-15]"
```
...
git push --force-with-lease origin ASP-15-stripe-checkout-integration
```
...
yarn lint:fix
git add .
git commit -m "style: fix linting issues [ASP-15]"
```
...
```bash
# Rename branch
git branch -m ASP-15-correct-format
git push origin -u ASP-15-correct-format
...
**Pipeline Issues**: cheddarfox (ARCHitect-in-the-IDE)  
**Team Coordination**: #cheddarfox-development
