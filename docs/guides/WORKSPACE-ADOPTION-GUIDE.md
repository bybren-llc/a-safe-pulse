...
git remote add harness https://github.com/bybren-llc/a-safe-pulse.git
git fetch harness
```
...
```
bybren-llc/
├── a-safe-pulse/             # Main application repo (PRIMARY)
│   ├── .claude/                  # Full harness
│   ├── dark-factory/             # Agent teams run here
│   └── ...
│
└── a-safe-pulse-packages/    # Shared packages repo (SECONDARY)
...
```bash
cd a-safe-pulse-packages
git remote add harness https://github.com/bybren-llc/a-safe-pulse.git
git fetch harness
git checkout harness/dev -- .claude/ .gemini/ .codex/ .cursor/ .agents/ CLAUDE.md AGENTS.md
...
cp -r ../a-safe-pulse/.claude/ .claude/
cp -r ../a-safe-pulse/.gemini/ .gemini/
cp -r ../a-safe-pulse/.codex/ .codex/
cp -r ../a-safe-pulse/.cursor/ .cursor/
cp -r ../a-safe-pulse/.agents/ .agents/
cp ../a-safe-pulse/CLAUDE.md .
cp ../a-safe-pulse/AGENTS.md .

# Adjust project-specific values
sed -i "s/a-safe-pulse/a-safe-pulse-packages/g" CLAUDE.md
```
...
2. **Multiple sessions** — Run separate dark factory sessions for each repo:
   ...
   # Session 1: workspace
   cd a-safe-pulse
   ./dark-factory/scripts/factory-start.sh feature ASP-10

   # Session 2: packages (in another terminal)
   cd a-safe-pulse-packages
   ./dark-factory/scripts/factory-start.sh story ASP-11
   ```
...
git clone git@github.com:bybren-llc/a-safe-pulse.git
cd a-safe-pulse
...
```bash
FACTORY_PROJECT_DIR="/home/{{REMOTE_USER}}/a-safe-pulse"
FACTORY_MAIN_BRANCH="dev"
FACTORY_TICKET_PREFIX="ASP"
...
cd /home/{{REMOTE_USER}}
git clone git@github.com:bybren-llc/a-safe-pulse-packages.git

# Start a separate factory session pointing to packages
cd a-safe-pulse-packages
# Temporarily override FACTORY_PROJECT_DIR
FACTORY_PROJECT_DIR="$(pwd)" ./dark-factory/scripts/factory-start.sh story ASP-11
```
...
git diff HEAD harness/dev -- CLAUDE.md
git diff HEAD harness/dev -- .claude/team-config.json
```
...
cd a-safe-pulse
git fetch harness && git checkout harness/dev -- .claude/ .gemini/ .codex/ .cursor/ .agents/
bash scripts/setup-template.sh
