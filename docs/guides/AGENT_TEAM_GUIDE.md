...
The ASP project uses a SAFe Agile Release Train (ART) agent team powered by Claude Code.
...
4. **HITL Model**: Scott Graham (POPM) has final approval on all deliverables
...
├── asp-228-planning.md           # Planning doc example
├── spec-asp-135-posthog-analytics-safe.md  # Implementation spec example
...
4. Test with `cheddarfox_app_user` (not superuser)
...
4. **DE**: Test locally with `cheddarfox_app_user`
...
### To POPM (Scott Graham) - HITL Model
