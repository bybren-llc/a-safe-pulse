...
3. **Apply substitutions** -- Replaces `{{PLACEHOLDER}}` tokens in upstream
   files with your manifest's identity values (e.g., `ASP`
   becomes `REN`).
...
Upstream repo                    Your fork
  ...
  commands/                        commands/
    start-work.md   ---substitute-->  start-work.md (ASP -> SCA)
```
...
2. Keys are sorted by length descending (longest-match-first) to prevent
   partial matches. For example, `https://github.com/bybren-llc/a-safe-pulse` is replaced before
   `bybren-llc`.
...
[ERROR] Preflight failed: 2 issue(s) found
  ...
  commands/start-work.md:12: unreplaced token a-safe-pulse
