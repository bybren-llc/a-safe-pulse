...
```
I need to implement ASP-42 (User Profile Management).
The spec is at specs/ASP-42-user-profile-spec.md.
...
```
I need a thorough review of the changes on branch ASP-99-payment-flow.
...
- For Stories where the implementer can handle the PR (RTE role collapsed per ASP-499), a team of 2 (implementer + QAS) is often sufficient.
...
if [ "$TEAMMATE_ROLE" = "qas" ]; then
  ...
  if [ ! -f "test-results/ASP-latest.json" ]; then
...
./dark-factory/scripts/factory-start.sh feature ASP-42
