...
git checkout ASP-XXX-your-feature
...
grep -A 30 "Low-Level Tasks" specs/ASP-XXX-{feature}-spec.md

# Check for security-critical requirements
grep "#EXPORT_CRITICAL" specs/ASP-XXX-{feature}-spec.md

# Check for architectural decisions that affect your work
grep "#PATH_DECISION" specs/ASP-XXX-{feature}-spec.md
```
...
3. **Branch is created** (if one does not exist):
   ...
   ```bash
   git checkout -b ASP-XXX-short-description
   ```
...
# Should show: ASP-XXX-short-description

# Confirm spec is available
ls specs/ASP-XXX-*-spec.md
```
...
```
@system-architect I need to implement [feature] for ASP-XXX.
...
git commit -m "feat(api): add user profile endpoint with RLS context [ASP-123]"

# Bug fix
git commit -m "fix(ui): resolve login redirect loop on expired session [ASP-456]"

# Test addition
git commit -m "test(api): add integration tests for profile endpoint [ASP-123]"

# Multi-line commit for complex changes
git commit -m "$(cat <<'EOF'
feat(payments): add Stripe checkout flow with webhook handling [ASP-789]
...
- [ ] **Rebase is clean**: `git rebase origin/dev` completes without conflicts
...
- [ ] **Commit messages follow format**: `type(scope): description [ASP-XXX]`
...
git push --force-with-lease origin ASP-XXX-your-feature
...
**Issue**: Rebase conflicts after `git rebase origin/dev`
...
- **Solution**: Verify you are using the correct context helper and the test database user is `cheddarfox_app_user`
...
- GitHub Discussions: https://github.com/bybren-llc/a-safe-pulse/discussions
