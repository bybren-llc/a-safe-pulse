...
grep -A 20 "Acceptance Criteria" specs/ASP-XXX-{feature}-spec.md
...
```markdown
## QAS Validation Report - ASP-XXX

**Session ID**: [Claude Code session ID]
**Date**: [YYYY-MM-DD]
**Spec**: specs/ASP-XXX-{feature}-spec.md
...
mcp__linear-mcp__get_issue "ASP-XXX"

# Read comments from implementers
mcp__linear-mcp__get_issue_comments "ASP-XXX"

# Check ticket history for context
mcp__linear-mcp__get_issue_history "ASP-XXX"
```
...
- **Action**: `ls specs/ | grep "ASP-XXX"` or ask `@bsa`
...
- **Solution**: Verify test database user is `cheddarfox_app_user`, not the superuser
