...
1. Visit: https://gitingest.com/bybren-llc/{{GITHUB_REPO}}
...
2. Create new issue
3. Title: `ASP-1: Add Hello World endpoint for agent workflow validation`
...
- [ ] Ticket has proper format (ASP-1)
...
```
I'm implementing Linear ticket ASP-1: Add Hello World endpoint.
...
```bash
git checkout -b ASP-1-hello-world-endpoint
git add .
git commit -m "feat(api): add Hello World endpoint [ASP-1]
...
Closes ASP-1"
git push origin ASP-1-hello-world-endpoint
```
...
# Using GitHub CLI
gh pr create \
  --title "feat(api): add Hello World endpoint [ASP-1]" \
  --body "Implements ASP-1
...
## Linear Ticket
https://linear.app/your-team/issue/ASP-1"
```
...
```bash
FACTORY_PROJECT_DIR="/path/to/a-safe-pulse"
...
./dark-factory/scripts/factory-start.sh story ASP-TEST
