# Knowledge Transfer Meta-Prompt: Harness Modernization ASP-444

## For the ARCHitect of a-safe-pulse

**Purpose**: Enable extraction and generalization of ASP harness improvements for the public repository
**Source Epic**: ASP-444 (Harness & Skills Modernization)
**Related Tickets**: ASP-454 (SOP Integration), ASP-455, ASP-456
**Date**: 2025-12-20
...
The ASP team completed a comprehensive harness modernization that:
...
```text
docs/whitepapers/CLAUDE-CODE-HARNESS-MODERNIZATION-ASP-444.md
```
...
- Replace `ASP-XXX` pattern with `ASP-XXX`
...
### What's ASP-Specific (Must Replace)

| Component        | ASP Value                    | Replace With                       |
| ---------------- | ----------------------------- | ---------------------------------- |
| Ticket prefix    | `ASP-`                        | `ASP-` or parameterize |
| Branch pattern   | `ASP-{number}-{desc}`         | `{{PREFIX}}-{number}-{desc}`         |
| Team name        | `ASP`                        | `a-safe-pulse`                   |
| Slack channels   | `#github-feed`                | `{{SLACK_CHANNEL}}`                  |
| Linear workspace | ASP workspace                | Generic Linear setup               |
| Docker registry  | `ghcr.io/bybren-llc/cheddarfox-app` | `{{REGISTRY}}/{{PROJECT}}`             |
| Database         | `linear_agent`, `postgres`       | `{{PROJECT}}_dev`, `{{PROJECT}}_user`  |
...
1. **Create SOP templates** based on ASP SOPs
...
### Commits on ASP-454 Branch

```text
fa4e942 docs(harness): integrate SOPs into whitepaper and AGENTS.md [ASP-454]
c1f19b7 docs(harness): add SAFe Role Expansion Vision to Phase 3 roadmap [ASP-454]
4d68d72 feat(harness): upgrade SAFe agents to Opus 4.5, add skills integration [ASP-454]
9b3beec docs(harness): update whitepaper with Phase 0-2 completion [ASP-454]
```

### Key Files Changed

- `AGENTS.md` - Role fixes, skills section, SOP links
- `docs/whitepapers/CLAUDE-CODE-HARNESS-MODERNIZATION-ASP-444.md` - Main whitepaper
...
```text
ASP Repository Structure (Source)
├── .claude/
...
├── docs/
│   ├── whitepapers/
│   │   ├── CLAUDE-CODE-HARNESS-MODERNIZATION-ASP-444.md
...
The public repo is "a practical and artistic expression" of ASP's work.
...
[a-safe-pulse](https://github.com/bybren-llc/a-safe-pulse) repository.
