...
**Impact**: Production incident ASP-221/ASP-222 (Sept 2025) - RLS policies not applied, 4+ hour emergency debugging
...
**Impact**: Production incident ASP-315 (Oct 2025) - Tables created WITHOUT RLS protection
...
- [ ] **Security Testing**
  ...
  psql -U cheddarfox_app_user -d linear_agent < scripts/test-new-table-rls.sql
  ```
...
# - Update "Recent Changes" section with ASP-XXX reference

# 3. Commit both schema and docs together
git add prisma/migrations/ docs/database/DATA_DICTIONARY.md
git commit -m "feat(db): add new table with documentation [ASP-XXX]"
```
...
CREATE POLICY [table_name]_app_isolation ON [table_name]
    FOR ALL TO cheddarfox_app_user
    USING (user_id = current_setting('app.current_user_id', true));
...
psql -U cheddarfox_app_user -d linear_agent << 'EOF'
...
This SOP is **mandatory** for all database changes in the a-safe-pulse application. Non-compliance may result in:
...
**Updated**: ASP-321 (Production incident learnings - separate RLS files, migrate resolve misuse)
