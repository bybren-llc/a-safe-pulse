...
This directory contains the ASP Claude Code harness: hooks, slash commands, and (coming soon) skills for workflow automation.

## Harness Architecture

```text
┌──────────────────────────────────────────────────────────────────────┐
│                      ASP Claude Code Harness                         │
├──────────────────────────────────────────────────────────────────────┤
│                                                                       │
...
- **ASP Round Table**: humans + AI agents are peers; Stop-the-Line authority is encouraged

Canonical reference: `.cursor/rules/06-team-culture.mdc`

## Role Execution Modes (ASP-499)
...
| HITL                      | Final Authority         | ❌ Never        | Scott Graham merges                         |
...
| ---------------------- | ------------------ | ---------------------- |
| `/check-docker-status` | `/remote-status`   | Deprecated per ASP-445 |
| `/deploy-dev`          | `/remote-deploy`   | Deprecated per ASP-445 |
| `/dev-health`          | `/remote-health`   | Deprecated per ASP-445 |
| `/dev-logs`            | `/remote-logs`     | Deprecated per ASP-445 |
| `/rollback-dev`        | `/remote-rollback` | Deprecated per ASP-445 |
...
## Dual-Mode Deployment (ASP-445 Terminology Contract)

Pop OS supports two deployment modes. **Use canonical terminology:**

| Mode        | Container Name     | Port | Use Case                     |
| ----------- | ------------------ | ---- | ---------------------------- |
| **Dev**     | `app`     | 3000 | Daily development (STANDARD) |
| **Staging** | `asp-staging` | 3001 | Release validation/UAT       |
...
# git add . && git commit -m "feat(scope): description [ASP-347]"
...
# git push --force-with-lease origin ASP-347-branch
# # Use the PR template as your PR body baseline:
# # gh pr create --title "feat(scope): description [ASP-347]" --body "$(cat .github/pull_request_template.md)"
# gh pr create ...
...
| Trigger               | Behavior                                                                      |
| --------------------- | ----------------------------------------------------------------------------- |
| On prompt submit      | Reminds about `ASP-{number}` branch naming convention                         |
...
**Maintained by**: ASP Development Team + ARCHitect-in-the-IDE (Auggie)
