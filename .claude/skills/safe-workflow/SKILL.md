...
- User mentions starting work on a ticket (e.g., "I'm starting ASP-447")
...
- MUST start with `ASP-` followed by ticket number
...
Every commit MUST end with `[ASP-XXX]` referencing the ticket.
...
refactor(db): optimize query performance [ASP-234]
chore: upgrade dependencies [ASP-337]
```
...
# All commits must follow SAFe format with [ASP-XXX]
```
