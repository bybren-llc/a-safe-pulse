...
### NEW (ASP-314): Production Deployment Owner
...
cat specs/ASP-XXX-{feature}-spec.md | grep -A 30 "Pull Request Template"

# Extract logical commits
grep -r "Logical Commits|git commit" specs/ASP-XXX-spec.md

# Get demo script for validation
grep -r "Demo Script" specs/ASP-XXX-spec.md
```

### 5. Review Documentation

- `CONTRIBUTING.md` - Complete workflow (MANDATORY)
- `specs/ASP-XXX-{feature}-spec.md` - Implementation spec with PR template
...
git branch --show-current | grep -E "^ASP-[0-9]+-" && echo "✅ Branch name valid"

# 2. Verify commit message format
git log --oneline -1 | grep -E "^[a-z]+(\([a-z]+\))?: .+ \[ASP-[0-9]+\]" && echo "✅ Commit format valid"
...
- [ ] Commits follow SAFe format: `type(scope): description [ASP-XXX]` ✅
...
3. Select base: `dev` and compare: `ASP-{number}-{description}`
...
git commit -m "fix(ci): resolve test failure [ASP-XXX]"
...
git add .
git commit -m "refactor(scope): address PR feedback [ASP-XXX]"
...
**You do NOT merge** - Scott Graham (or designated HITL) is final merge authority.
...
> "PR #XXX for ASP-YYY is Ready for HITL Review. All CI green, reviews complete, evidence attached. Awaiting final merge approval from Scott Graham."

**Notify Scott Graham** and wait for merge.
...
git log --oneline -5 | grep "ASP-XXX"

# Linear ticket auto-sync:
# - Tickets referenced in commit messages (e.g., [ASP-XXX]) auto-move to Done
...
- SAFe commit format: `type(scope): description [ASP-XXX]`
...
- PR Number: #XXX
- Title: feat(scope): description [ASP-XXX]
- Base: dev
- Compare: ASP-XXX-description
...
# 2. Rebase and push
git fetch origin && git rebase origin/dev
git push --force-with-lease origin ASP-123-feature

# 3. Create PR
gh pr create --title "feat(feature): implement feature [ASP-123]" --web

# 4. Monitor CI
gh pr checks

# 5. Handoff to HITL (RTE does NOT merge)
# Notify Scott Graham: "PR #XXX ready for HITL review"
...
```bash
# Agent A (FE): ASP-123-ui-component (depends on ASP-124)
# Agent B (BE): ASP-124-api-endpoint (must merge first)

# RTE coordinates (but does NOT merge):
# 1. Notify HITL: "ASP-124 ready, blocks ASP-123"
# Wait for Scott to merge ASP-124 via GitHub

# 2. After HITL merges ASP-124, RTE rebases ASP-123
git checkout ASP-123-ui-component
git fetch origin && git rebase origin/dev
git push --force-with-lease

# 3. Notify HITL: "ASP-123 ready after ASP-124 merged"
# RTE work ends here - Scott handles merge via GitHub
```
...
- **No Merge**: You prepare for merge, HITL (Scott Graham) does the merge
...
4. **Handoff Statement**
   > "PR #XXX for ASP-YYY is Ready for HITL Review. CI green, reviews complete, evidence attached."
