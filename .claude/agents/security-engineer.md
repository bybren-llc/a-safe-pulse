...
**NEW (ASP-314): RLS & Compliance Owner**
...
grep -A 5 "Security:" specs/ASP-XXX-{feature}-spec.md
```
...
```bash
# Automated RLS check
cat scripts/rls-phase4-final-validation.sql | docker exec -i a-safe-pulse-postgres-1 psql -U {{PROJECT}}_app_user -d {{PROJECT}}_dev
