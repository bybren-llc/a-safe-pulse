    ...
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "git branch --show-current 2>/dev/null | grep -q '^ASP-[0-9]' || echo '⚠️ REMINDER: Branch should follow ASP-{number}-{description} format.'",
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "echo '📝 REMINDER: Commit message must follow SAFe format: type(scope): description [ASP-XXX]'",
...
**Note**: Customize the ticket prefix (`ASP-`) and branch names (`dev`/`master`) for your project.
...
| Find             | Replace With                       |
| ---------------- | ---------------------------------- |
| `ASP-`           | Your ticket prefix (e.g., `PROJ-`) |
| `a-safe-pulse` | Your project name                  |
...
| Command                           | Purpose                       |
| --------------------------------- | ----------------------------- |
| `/start-work ASP-123` | Begin work on a ticket        |
