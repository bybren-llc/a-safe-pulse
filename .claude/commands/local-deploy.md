...
| ------------------------- | ----------- | ---------------------------- | ---------------------------------------------------------------------------- |
| **Development** (default) | (none)      | `a-safe-pulse-dev-app`     | Active development with hot-reload (STANDARD port 3000)                      |
| **Staging**               | `--staging` | `a-safe-pulse-staging-app` | Production-like, self-contained, survives git operations (port 3001)         |
| **Both**                  | `--both`    | Both containers              | Run dev and staging simultaneously (ASP-400/ASP-401) |
...
docker ps --filter name=a-safe-pulse-staging-app --format '{{.Names}}\t{{.Status}}'

# Check for dev container (ASP-400: updated container name)
docker ps --filter name=a-safe-pulse-dev-app --format '{{.Names}}\t{{.Status}}'
```

Get current commit SHA:

```bash
# Try staging first, then dev (ASP-400: updated container names)
docker inspect a-safe-pulse-staging-app 2>/dev/null | grep 'org.opencontainers.image.revision' | cut -d'"' -f4 || \
docker inspect a-safe-pulse-dev-app 2>/dev/null | grep 'org.opencontainers.image.revision' | cut -d'"' -f4
```
...
```bash
# For dev mode (STANDARD port 3000, ASP-401)
docker ps --filter name=a-safe-pulse-dev --format 'table {{.Names}}\t{{.Status}}'

# For staging mode (port 3001)
docker ps --filter name=a-safe-pulse-staging --format 'table {{.Names}}\t{{.Status}}'
```
...
```bash
# Staging
docker inspect a-safe-pulse-staging-app | grep 'org.opencontainers.image.revision' | cut -d'"' -f4

# Dev (ASP-400: updated container name)
docker inspect a-safe-pulse-dev-app | grep 'org.opencontainers.image.revision' | cut -d'"' -f4
```
...
```bash
# Staging (ASP-400: use container name)
docker logs a-safe-pulse-staging-app --tail 50

# Dev (ASP-400: use container name or script)
docker logs a-safe-pulse-dev-app --tail 50
# Or use script
...
Current:  e9722d4 - style(docs): apply markdown linting fixes [ASP-347]
Status:   Up 7 hours (healthy)

Target:   3a49b85 - feat(ci): add Slack notifications [ASP-350]
...
Running:  3a49b85 - feat(ci): add Slack notifications [ASP-350]
Status:   Healthy
...
## Running Both Modes Simultaneously (ASP-400/ASP-401)
...
```bash
docker ps --filter name=a-safe-pulse --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'
```
