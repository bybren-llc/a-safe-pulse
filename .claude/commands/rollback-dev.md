...
```bash
# ASP-445: Updated container name per Terminology Contract
...
```bash
ssh -i {{SSH_KEY_PATH}} {{REMOTE_USER}}@{{REMOTE_HOST}} "docker images {{REGISTRY}}/a-safe-pulse/dev --format 'table {{.Tag}}\t{{.ID}}\t{{.CreatedAt}}'"
```
...
```bash
ssh -i {{SSH_KEY_PATH}} {{REMOTE_USER}}@{{REMOTE_HOST}} "docker pull {{REGISTRY}}/a-safe-pulse/dev:dev-{target-sha}"
```
...
```bash
# From: image: {{REGISTRY}}/a-safe-pulse/dev:latest
# To:   image: {{REGISTRY}}/a-safe-pulse/dev:dev-{target-sha}
```

Use SSH + sed for inline replacement:

```bash
ssh -i {{SSH_KEY_PATH}} {{REMOTE_USER}}@{{REMOTE_HOST}} "cd {{PROJECT_PATH}} && sed -i 's|{{REGISTRY}}/a-safe-pulse/dev:latest|{{REGISTRY}}/a-safe-pulse/dev:dev-{target-sha}|g' docker-compose.dev.yml"
```
...
Version:  3a49b85 - feat(ci): add Slack notifications [ASP-350]
...
Selected: e9722d4 - style(docs): apply markdown linting fixes [ASP-347]
...
Running:  e9722d4 - style(docs): apply markdown linting fixes [ASP-347]
Status:   ✅ Healthy
...
ssh -i {{SSH_KEY_PATH}} {{REMOTE_USER}}@{{REMOTE_HOST}} "cd {{PROJECT_PATH}} && sed -i 's|{{REGISTRY}}/a-safe-pulse/dev:dev-[a-f0-9]*|{{REGISTRY}}/a-safe-pulse/dev:latest|g' docker-compose.dev.yml"
...
- Check available tags: `docker images | grep a-safe-pulse`
...
| `{{PROJECT_PATH}}`  | Project directory on remote | `~/Projects/a-safe-pulse`, `~/app` |
| `{{REGISTRY}}`      | Container registry URL      | `ghcr.io/bybren-llc`                 |
| `a-safe-pulse`  | Project name in registry    | `myapp`, `webapp`                    |
