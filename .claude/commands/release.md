...
> **📋 TEMPLATE**: This command uses `dev`, `ASP`, and `bybren-llc/{{GITHUB_REPO}}` placeholders. Replace with your project values.
...
git fetch origin
git log --oneline origin/dev..HEAD  # Must be empty (in sync)
```
...
gh pr merge <NUMBER> --squash --subject "type(scope): description [ASP-XXX]"
```
...
git push origin dev:<SECONDARY_BRANCH>
```

Verify sync:

```bash
git log --oneline origin/dev..origin/<SECONDARY_BRANCH>  # Should be empty
git log --oneline origin/<SECONDARY_BRANCH>..origin/dev  # Should be empty
```
...
gh api repos/bybren-llc/{{GITHUB_REPO}}/tags --jq '.[0:5] | .[].name'
```
...
gh api repos/bybren-llc/{{GITHUB_REPO}}/branches --jq '.[].name' | grep -v '^main$\|^dev$'
```

For each stale branch from a merged PR:

```bash
gh api -X DELETE repos/bybren-llc/{{GITHUB_REPO}}/git/refs/heads/<BRANCH_NAME>
```
...
echo "=== Remote ===" && gh api repos/bybren-llc/{{GITHUB_REPO}}/branches --jq '.[].name'
...
- Local working tree is clean on dev
...
| `ASP` | Linear/Jira ticket prefix | `WOR`, `SCA`, `PROJ` |
