...
-- 5. Create RLS policy for superuser (postgres)
...
-- 6. Create RLS policy for application user (cheddarfox_app_user)
...
DATABASE_URL="postgresql://postgres:postgres@localhost:5432/linear_agent" \
npx prisma migrate dev --name add_user_preferences_with_rls
...
- 2025-10-03: Created table with RLS policies [ASP-300]
```
...
# Test user isolation
psql -U cheddarfox_app_user -d linear_agent \
