...
COMMANDS_DIR = Path("/home/cheddarfox/Projects/a-safe-pulse/.claude/commands")
...
def process_workflow_command(filepath: Path):
    ...
    # Replace ASP- with {TICKET_PREFIX}-
    content = content.replace("ASP-", "{TICKET_PREFIX}-")
...
def process_rollback_dev(filepath: Path):
    ...
    patterns = {
        ...
        "~/Projects/cheddarfox-team": "{PROJECT_PATH}",
        "ghcr.io/bybren-llc/cheddarfox-app/dev": "{REGISTRY}/{PROJECT_NAME}/dev",
        "app": "{PROJECT}-dev-app",
        "name=cheddarfox": "name={PROJECT}",
        "Pop OS": "{DEV_MACHINE}",
        "ASP-": "{TICKET_PREFIX}-",
    }
    content = replace_patterns(content, patterns)
    ...
    placeholders = [
        ...
        ("{PROJECT_PATH}", "Project directory on remote", "`~/Projects/cheddarfox-team`, `~/app`"),
        ("{REGISTRY}", "Container registry URL", "`ghcr.io/bybren-llc`"),
        ("{PROJECT_NAME}", "Project name in registry", "`cheddarfox-app`, `myapp`"),
        ("{PROJECT}", "Short project identifier", "`cheddarfox`, `myapp`"),
