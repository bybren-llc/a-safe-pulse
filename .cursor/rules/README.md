...
| File                     | Globs                                      | Purpose                                |
|--------------------------|--------------------------------------------|----------------------------------------|
| `12-database-sql.mdc`    | `src/db/**, src/db/migrations/**`          | PostgreSQL, raw SQL migrations, RLS    |
| `13-testing.mdc`          | `tests/**/*.ts, tests/**`                  | Jest, unit and integration tests       |
| `14-spec-creation.mdc`   | `specs/**/*.md, specs_templates/**/*.md`    | Spec templates, AC/DoD, pattern refs   |
| `15-deployment.mdc`       | `docker-compose*.yml, Dockerfile*, scripts/deploy*` | Deploy procedures, Docker |
...
| `21-agent-backend.mdc`     | BE Developer: Node.js/Express/TypeScript API implementation, RLS enforcement |
...
The following upstream harness rules were not applicable to this project's tech stack and have been moved to `docs/archive/harness-upstream/cursor-rules/`:

| Archived File                | Reason                                    |
|------------------------------|-------------------------------------------|
| `10-backend-python.mdc`     | No Python in this project                 |
| `11-frontend-react.mdc`     | No frontend in this project               |
| `16-payments.mdc`           | No payment integration in this project    |
...
These activate when you open or edit files matching their glob pattern. For example, editing `src/db/migrations/008_new_table.sql` will automatically load `12-database-sql.mdc`.
...
@21-agent-backend Implement the API endpoint from spec ASP-123
...
```
@30-background-agents Set up a background agent for ASP-42
...
3. **Template-Ready**: Uses `ASP`, `dev`, and other placeholders for project customization
4. **Hierarchical**: Numbering (00-02, 12-15, 20-23, 30-31) groups rules by activation type
