...
# Or use Linear CLI:
linear issue update ASP-XXX --state "Done"
```
...
# Or use Linear CLI:
linear issue comment ASP-XXX "**Dev Evidence**\n\n..."
```
...
Tickets referenced in commit messages (e.g., `[ASP-123]`) automatically move to **Done** when the PR merges. Child stories not referenced in any commit message must be manually closed after merge.
...
- Branch name contains `ASP-XXX`
- PR title contains `[ASP-XXX]`
