...
This directory contains the master templates for the spec-driven workflow, which is the process for defining work in ASP projects. All implementation tasks are guided by a detailed, version-controlled specification file (`spec.md`) that serves as the single source of truth.
...
The master template for a single unit of work (typically a Linear User Story). The BSA agent copies this template to create a new `ASP-XXX-feature-name-spec.md` file for each task.
